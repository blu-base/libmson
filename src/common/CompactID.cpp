...
uint8_t CompactID::getN() const
{
  return n;
}

void CompactID::setN(const uint8_t& value)
{
  n = value;
}

uint32_t CompactID::getGuidIndex() const
{
  return guidIndex;
}
...
ExtendedGUID* CompactID::getExtendedGUID() const
{
  return eguid;
}

void CompactID::setExtendedGUID(ExtendedGUID* const guid)
{
  eguid = guid;
}
...
{
  return n == eguid->data_n
      && guidIndex < 0xFFFFFF;
}

bool CompactID::isNull() const
{
  return (n == 0) && (guidIndex == 0);
}
...
QString CompactID::toString() const
{
  return QString(QString::fromStdString(
                   std::to_string(guidIndex) + ", " + std::to_string(n)
                   ));
}
...
bool operator==(const CompactID& lhs, const CompactID& rhs) noexcept
{
  return lhs.getN() == rhs.getN()
      && lhs.getGuidIndex() == rhs.getGuidIndex();
}
...
CompactID::CompactID()
  : n {0}, guidIndex {0}, eguid{nullptr}
{
}

CompactID::CompactID(ExtendedGUID* guid)
  : n {0}, guidIndex {0}, eguid {guid}
{
...
CompactID::CompactID(const uint8_t n, const uint32_t guidIndex, ExtendedGUID* guid)
  : n{n}, guidIndex{0}, eguid{guid}
{
  setGuidIndex(guidIndex);
}

CompactID::CompactID(const QByteArray& bytes)
  : n{0}, guidIndex {0}
{
  setN(bytes.at(0));
