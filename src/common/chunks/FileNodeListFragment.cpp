...
FileNodeListFragment::FileNodeListFragment()
  : m_fileNodeListID(), m_nFragmentSequence(0), m_paddingLength(0) {}

std::list<std::shared_ptr<RevisionStoreChunkContainer>>
FileNodeListFragment::fileNodes() const {
  return m_fileNodes;
}

std::list<std::shared_ptr<RevisionStoreChunkContainer>> &
FileNodeListFragment::getFileNodes() {
  return m_fileNodes;
}

void FileNodeListFragment::setFileNodes(
    const std::list<std::shared_ptr<RevisionStoreChunkContainer>> &fileNodes) {
  m_fileNodes = fileNodes;
}

std::weak_ptr<RevisionStoreChunkContainer> FileNodeListFragment::getNextFragment() {
