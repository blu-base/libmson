...
void PropertyType_EightBytesOfData::generateXml(QXmlStreamWriter& xmlWriter) const
{
...
void PropertyType_EightBytesOfData::deserialize(QDataStream &ds) {
    ...
    char* rawBody = new char[8];
    ds.readRawData(rawBody, 8);

    m_data = QByteArray(QByteArray::fromRawData(rawBody, 8));
}
